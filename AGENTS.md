# AGENTS.md — invoicing-platform

## Scope and read order

Canonical instructions for coding agents working in **invoicing-platform**. Read this file before editing.
...
6. **Report with "How & Why"** per §0.1, and update `LESSONS.md` if you learned something
   non-obvious.
7. **When requested, commit** with a clear message (see Git and delivery).

Do not batch ten items and a giant commit. One coherent unit, explained, then the next.

## Project and product rules

An extensible, transactional invoicing platform built as a learning project. Read [README.md](README.md) for product scope and [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for its design.
...
- [ ] If requested, it is committed with a clear message.
